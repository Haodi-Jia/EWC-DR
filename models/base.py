...
class BaseLearner(object):
    ...
    def eval_task(self, save_conf=False):
        ...
        if self.args["maha"]:
            ...
            nme_accy = maha_accy
            self.pre_dict[self._cur_task] = y_pred
            self.target_dict[self._cur_task] = y_true
        else:
        """
        self.pre_dict[self._cur_task] = y_pred
        self.target_dict[self._cur_task] = y_true
        ...
        if self.args["model_name"] == "fecam":
            ...
            nme_accy = self._evaluate(y_pred, y_true)  # maha_accy
    ...
    def _eval_maha(self, loader, init_means, class_means):
        ...
        scores = dists.T  # [N, nb_classes], choose the one with the smallest distances
    ...
    def _mahalanobis(self, vectors, class_means, cov=None):
        ...
        x_minus_mu = F.normalize(vectors, p=2, dim=-1) - F.normalize(class_means.to(self._device), p=2, dim=-1).to(torch.float32)
        if cov is None:
